...
export const ShowError: ErrorRouteComponent = (props) => {
  ...
  const isNotFound = props.error instanceof FetchError && props.error.status === 404
  const isAPIError = props.error instanceof FetchError && props.error.status !== 404
  if (isNotFound) {
    //@ts-expect-error
  ...
  return (
    <main className="page space-y-9">
      ...
      <p className="max-w-lg text-center">
        ...
        does not work, try going Home. If that does not work, reopen the app.
      </p>
